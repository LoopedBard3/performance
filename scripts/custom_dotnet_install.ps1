...
function Is-Dotnet-Package-Installed([string]$InstallRoot, [string]$RelativePathToPackage, [string]$SpecificVersion) {
    ...
    if ($null -ne $results -and $results.PSobject.Properties.Name -contains "Count" -and $results.Count -gt 0) {
        ...
        foreach ($result in $results) {
            Say-Verbose "Printing result.FullName: $($result.FullName)"
        }
    }
    else {
        Say-Verbose "No results found."
    }
    Say-Verbose "Is-Dotnet-Package-Installed: Done printing out $InstallRoot contents"
    $DotnetPackagePath = Join-Path -Path $InstallRoot -ChildPath $RelativePathToPackage | Join-Path -ChildPath $SpecificVersion
    Say-Verbose "Is-Dotnet-Package-Installed: DotnetPackagePath=$DotnetPackagePath; Returning $(Test-Path $DotnetPackagePath -PathType Container)"
